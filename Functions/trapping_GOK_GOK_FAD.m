% This model uses General order kinetic (GOK) for trapping and detrapping.
% GOK assumes trapping slower than SSE, due to e.g., Coulomb repulsive
% forces. Also, the effective trap lifetime vary with time. (Guralnik et
% al., 2015)
% A slower than exponential thermal detrappign is explained by electron
% retrapping or distant-dependent probabilities of detrapping.
% Athermal fading is accounted for and s_ath = 3e15 (Huntley, 2006)
%%maximebernard%%
% last update: 27/01/2025

function [nNf] = trapping_GOK_GOK_FAD(time,temp,kparams);
